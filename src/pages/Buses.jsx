...
const Buses = () => {
  ...
  const { user } = useAuth();
  const { setSelectedbus, setCurrentStep } = useBooking();
  ...
  }, [searchFilters, sortBy, buses, currentPage, itemsPerPage])
  ...
  const totalFilteredCount = useMemo(() => {
    ...
    return list.length;
  }, [searchFilters, sortBy, buses]);

  const handleBook = (bus) => {
    if (!user) {
      alert("Please login first to book a bus!");
      navigate('/login');
      return;
    }
  ...
  return (
    <>
      ...
      <SortBar
        filteredbusesLength={filteredbuses.length}
      ...
      >
        {filteredbuses.length === 0 ? (
          <div className="rounded-2xl bg-slate-900/60 border border-slate-700 px-4 py-8 text-center text-slate-300 text-sm">
            No buses found for this search. Try changing date or route.
          </div>
        ) : (
