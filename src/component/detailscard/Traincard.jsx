...
const CheapestTrainsDeals = () => {
    ...
    const fetchTrains = async () => {
        try {
            ...
            setTrains(res.data.trains || res.data.Trains || res.data);
        } catch (err) {
            console.error('API Error:', err);
            setTrains([
                { id: 1, from: 'Delhi', to: 'Agra', price: '₹499', image: 'https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?w=400&h=300&fit=crop', duration: '4h 15m 🚆', location: 'AC 2 Tier • 2h ago' },
                { id: 2, from: 'Mumbai', to: 'Pune', price: '₹350', image: 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=300&fit=crop', duration: '3h 30m 🚆', location: 'Express • 1h ago' },
                { id: 3, from: 'Bangalore', to: 'Chennai', price: '₹650', image: 'https://images.unsplash.com/photo-1578731297937-8ea40ee53827?w=400&h=300&fit=crop', duration: '6h 45m 🚆', location: 'Sleeper Class • 4h ago' },
                { id: 4, from: 'Chennai', to: 'Hyderabad', price: '₹799', image: 'https://images.unsplash.com/photo-1569718212165-3a8278d5f624?w=400&h=300&fit=crop', duration: '11h 20m 🚆', location: 'AC 3 Tier • 3h ago' },
                { id: 5, from: 'Delhi', to: 'Chandigarh', price: '₹299', image: 'https://images.unsplash.com/photo-1436491865332-7a61a109cc05?w=400&h=300&fit=crop', duration: '5h 10m 🚆', location: 'Shatabdi • 5h ago' }
            ]);
        } finally {
            setLoading(false);
        }
    ...
    // Filter trains by city
    const filteredTrains = selectedCity 
        ? trains.filter(train => 
            train.from.includes(selectedCity) || train.to.includes(selectedCity)
          )
        : trains;
    ...
    if (loading) return <div className="py-20 text-center text-gray-500">Loading Trains...</div>;
            ...
            <div className="max-w-7xl mx-auto relative rounded-3xl">
                ...
                <div className='flex justify-between items-center py-10'>
                    <h2 className="text-2xl font-bold text-gray-900 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">
                        🚆 Cheapest Train Deals ({filteredTrains.length})
                    </h2>
                ...
                {filteredTrains.length === 0 ? (
                    <div className="text-center py-20 text-gray-500">
                        No trains available for {selectedCity || 'selected filters'}.{' '}
                        <button 
...
export default CheapestTrainsDeals;
